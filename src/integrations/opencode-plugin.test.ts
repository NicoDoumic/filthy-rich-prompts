...
describe("opencode-plugin", () => {
  ...
  it("refines only text parts of parts-form messages", async () => {
    ...
    const textPart = {
      type: "text",
      text: "can you make it faster? using react btw",
    };
  ...
  it("no-ops on unexpected payloads without throwing", async () => {
    ...
    await expect(
      hook({}, { messages: [{ role: "assistant", content: "hi" }] }),
    ).resolves.toBeUndefined();
    await expect(
      hook(null, undefined as unknown as { messages?: unknown }),
    ).resolves.toBeUndefined();
  });
  ...
  it("does not throw when message object is frozen", async () => {
    ...
    const message = Object.freeze({
      role: "user",
      content: "fix the crash on export",
    });
