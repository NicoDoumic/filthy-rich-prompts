...
function readAutoRefine(path: string): { autoRefine: boolean } | { warning: string } {
  let raw: string;
  try {
  ...
  try {
    ...
    return { warning: `${path} is not valid JSON (strict mode in the pre-release; JSONC lands in M2)` };
  }
}
...
export function loadMinConfig(cwd: string, userConfigPath?: string): MinConfig {
  ...
  if ('autoRefine' in project) return { autoRefine: project.autoRefine, source: 'project' };
  if (project.warning !== 'unreadable') {
    return { ...DEFAULTS, warning: project.warning };
  }

  const userPath =
    userConfigPath ?? join(homedir(), '.config', 'filthy-rich-prompts', 'refine.config.json');
  const user = readAutoRefine(userPath);
  if ('autoRefine' in user) return { autoRefine: user.autoRefine, source: 'user' };
