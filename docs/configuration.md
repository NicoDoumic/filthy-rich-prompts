...
> Status: **Implemented** (4-level precedence, mode, pass toggles, output flags, `includeOriginal`, `minQuestions` — see `src/integrations/config-loader.ts`). Pass *options* (e.g. `{ structure: { style: "markdown" } }`) and per-model config remain future. Schema freezes at 1.0. Until then, additive changes only.
...
> **Implementation note:** The full 4-level precedence merge is live in `src/integrations/config-loader.ts`. Programmatic `RefineOptions` (the CLI/skill invocation level) win over project config, which wins over user config, which wins over defaults.
