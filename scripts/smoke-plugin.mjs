...
try {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "frp-plugin-smoke", type: "module" }),
  );
  ...
  const message = {
    role: "user",
    content: "can you make it faster? using react btw",
  };
  await transform({}, { messages: [message] });
  if (!message.content.includes("## Context")) {
    throw new Error(
      `hook did not refine the message: ${JSON.stringify(message.content.slice(0, 80))}`,
    );
  }

  console.log(
    "✓ Plugin bundle smoke test passed (standalone import + hook refines).",
  );
} finally {
